...
class Command(BaseCommand):
    help = "MVP uchun barcha asosiy modellarga boshlang'ich (demo) ma'lumot yaratadi."
    ...
    def handle(self, *args, **options):
        ...
        self.seed_plans()
        self.seed_math()
        self.seed_practice_tools()
        demo_user = self.seed_demo_user()
        self.seed_user_dependent(demo_user)
        self.stdout.write(self.style.SUCCESS("Barcha modellarga seed data muvaffaqiyatli qo'shildi."))
    ...
    def seed_subjects(self):
        subjects_data = [
            ...
            {"code": "russian", "name": "Rus tili", "kind": Subject.Kind.LANGUAGE, "is_active_in_mvp": True},
            {"code": "german", "name": "Nemis tili", "kind": Subject.Kind.LANGUAGE, "is_active_in_mvp": True},
            {"code": "turkish", "name": "Turk tili", "kind": Subject.Kind.LANGUAGE, "is_active_in_mvp": True},
        ]
        for data in subjects_data:
            subject, created = Subject.objects.get_or_create(code=data["code"], defaults=data)
            # Seed qayta ishga tushirilganda ham mavjud fanlar 5 talik katalogda faol bo'lsin.
            if subject.is_active_in_mvp != data["is_active_in_mvp"]:
                subject.is_active_in_mvp = data["is_active_in_mvp"]
                subject.save(update_fields=["is_active_in_mvp"])
            if created:
    ...
    def seed_avatars(self):
        ...
        for data in avatars_data:
            ...
            if created:
                self.stdout.write(self.style.SUCCESS(f"Avatar: {avatar.name}"))
    ...
    def seed_math(self):
        ...
        for name in topics_data:
            ...
            topics[name] = topic
            if created:
                self.stdout.write(self.style.SUCCESS(f"MathTopic: {topic.name}"))
    ...
    def seed_demo_user(self):
        ...
        if created:
            ...
            user.save()
            self.stdout.write(self.style.SUCCESS(f"User: {user.username} (parol: demo12345)"))
        return user

    def seed_user_dependent(self, user):
        english = Subject.objects.filter(code="english").first()
        ...
        if created:
            self.stdout.write(self.style.SUCCESS("LearningSession: demo sessiya yaratildi"))
